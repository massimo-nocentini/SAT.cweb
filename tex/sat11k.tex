...
${}\{{}$\1\6
\&{register} \&{int} \\{au}${},\39\\{av},\39\\{aw},\39\|h,\39\|i,\39\|j,\39%
\\{jj},\39\|k,\39\\{kk},\39\|l,\39\\{ll},\39\|p,\39\\{pp},\39\|q,\39\\{qq},\39%
\|r,\39\|s,\39\\{cia},\39\\{cis},\39\\{ci};{}$\6
\&{register} \&{int} \|c${},\39\\{cc},\39\\{hh},\39\\{la},\39\\{lp},\39\\{ls},%
\39\\{ola},\39\\{ols},\39\\{tla},\39\\{tls},\39\\{tll},\39\\{sl},\39\\{su},\39%
\\{sv},\39\\{sw};{}$\6
\&{register} \&{int} \|t${},\39\\{tt},\39\|u,\39\\{uu},\39\\{v0},\39\|v,\39%
\\{vv},\39\|w,\39\\{ww},\39\|x,\39\|y,\39\\{xl},\39\\{pu},\39\\{aa},\39\\{ss},%
\39\\{pv},\39\\{ua},\39\\{va};{}$\7
...
\M{3\*}The default values of parameters below have been tuned for a broad
...
\B\4\D$\\{show\_big\_clauses}$ \5
\T{64}\C{ \PB{\\{verbose}} code to print all big guys at beginning }\par
...
\&{ullng} \\{imems}${},\39\\{mems}{}$;\C{ mem counts }\6
...
\&{float} \\{alpha}${}\K\T{0.001}{}$;\C{ magic constant for heuristic scores }\6
\&{float} \\{gamm}${}\K\T{0.20}{}$;\C{ magic ratio for the clause reduction
heuristic }\6
\&{int} \\{theta64}${}\K\T{25}{}$;\C{ the optimization parameter $theta$, times
64 }\6
\&{int} \\{levelcand}${}\K\T{600}{}$;\C{ preselected candidates times levels }\6
\&{int} \\{mincutoff}${}\K\T{30}{}$;\C{ don't cut off fewer than this many
candidates }\6
\&{int} \\{max\_prelook\_arcs}${}\K\T{5000}{}$;\C{ space available for arcs re
strong components }\6
\&{int} \\{dl\_max\_iter}${}\K\T{1}{}$;\C{ maximum iterations of double-look }\6
\&{float} \\{dl\_rho}${}\K\T{0.9998}{}$;\C{ damping factor for the double-look
trigger }\par
\As7\*, 24\*, 36, 48, 60, 67, 89, 108, 120, 124, 133, 141\ETs164\*.
\U2\*.\fi
...
$\gamma$ in the clause reduction heuristic scores \PB{\\{clause\_weight}[\|k]}.
\item{$\bullet$}
...
\\{memk\_max}<\T{2}\V\\{memk\_max}>\T{31}\V\\{alpha}\Z\T{0.0}\V\\{gamm}\Z\T{0}%
\V\\{theta64}<\T{0}\V\\{levelcand}\Z\T{0}\V\\{mincutoff}\Z\T{0}\V\\{max%
\_prelook\_arcs}\Z\T{0}\V\\{dl\_max\_iter}\Z\T{0}){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Usage:\ "}\|O\.{"s\ [v<n>]\ [c<n>]\ [h<}\)%
\.{n>]\ [b<n>]\ [s<n>]\ [d}\)\.{<n>]\ [m<n>]\ "},\39\\{argv}[\T{0}]);{}$\6
${}\\{fprintf}(\\{stderr},\39\.{"\ [H<n>]\ [g<f>]\ [a<f}\)\.{>]\ [t<n>]\ [p<n>]%
\ [q<}\)\.{n]\ [z<n>]"});{}$\6
...
${}\}{}$ \&{vchunk};\par
\As6, 26, 27\*, 28, 34, 35, 88, 107\ETs119.
\U2\*.\fi
...
\&{ullng} \\{bclauses};\C{ how many clauses are big (have more than two
literals)? }\6
\&{ullng} \\{bcells};\C{ how many occurrences of literals in big clauses? }\6
...
${}\\{exit}({-}\T{2});{}$\6
\4${}\}{}$\2\6
${}\\{hash}\K{}$(\&{tmp\_var} ${}{*}{*})\,\\{malloc}(\&{sizeof}(\&{tmp\_var})%
\LL\\{hbits});{}$\6
\&{if} ${}(\R\\{hash}){}$\5
${}\{{}$\1\6
...
${}\{{}$\1\6
\&{if} ${}(\|k\G\T{3}){}$\1\5
${}\\{bclauses}\PP,\39\\{bcells}\MRL{+{\K}}\|k;{}$\2\6
\&{if} ${}(\|k>\\{max\_clause}){}$\1\5
${}\\{max\_clause}\K\|k;{}$\2\6
\4${}\}{}$\2\6
${}\\{cells}\MRL{+{\K}}\|k{}$;\par
\Us9\ET10.\fi
...
${}\\{new\_chunk}\K{}$(\&{chunk} ${}{*})\,\\{malloc}(\&{sizeof}(\&{chunk}));{}$%
\6
\&{if} ${}(\R\\{new\_chunk}){}$\5
${}\{{}$\1\6
...
\M{24\*}Quite a few cooperating data structures are needed to do all these
things at
...
\PB{\\{cinx}} and~\PB{\\{kinx}}, which are indexes into two larger arrays
called
\PB{\\{cmem}} and~\PB{\\{kmem}},  govern this aspect of the problem: \PB{%
\\{cinx}[\|c]} tells where
the literals of clause~$c$ are listed in~\PB{\\{cmem}}, while \PB{\\{kinx}[%
\|l]} tells where
the clauses that contain a given literal~$l$ are listed in~\PB{\\{kmem}}.
...
implications }\6
\&{uint} ${}{*}\\{cmem},\39{*}\\{kmem}{}$;\C{ master arrays for \PB{\\{cinx}}
and \PB{\\{kinx}} data }\6
\&{tdata} ${}{*}\\{cinx},\39{*}\\{kinx}{}$;\C{ indexes into \PB{\\{cmem}} and %
\PB{\\{kmem}} for the big clause info }\6
\&{tpair} ${}{*}\\{bstack}{}$;\C{ holding place for big clauses that become
binary or unary }\6
\&{int} \\{bptr};\C{ the number of elements used in \PB{\\{bstack}} }\6
\&{int} \\{max\_use};\C{ the maximum number of times any literal occurs }\6
\&{tpair} ${}{*}\\{tmem}{}$;\C{ master array of blocks for \PB{\\{timp}} lists
}\6
\&{tdata} ${}{*}\\{timp}{}$;\C{ indexes into \PB{\\{tmem}} for lists of ternary
implications }\6
\&{uint} ${}{*}\\{freevar},\39{*}\\{freeloc}{}$;\C{ perm of the variables from
free to assigned }\6
...
numbers,
while others (like \PB{\\{bimp}} and \PB{\\{kinx}}) are indexed by literal
numbers. In order
...
\M{27\*}An entry in \PB{\\{cinx}} has two parts: \PB{\\{addr}} is the address
in \PB{\\{cmem}} where the
list of literals for a given clause begins; \PB{\\{size}} is initially the
length of
that list. When literals of a clause become true or false, the \PB{\\{size}}
field
is adjusted in a somewhat tricky way, explained below within the \PB{%
\\{sanity}}
routine. The literals of the input clauses are loaded backwards into~\PB{%
\\{cmem}},
so that we have \PB{$\\{cinx}[\|c].\\{addr}+\\{cinx}[\|c].\\{size}\K\\{cinx}[%
\|c-\T{1}].\\{addr}$} when
computation begins.

An entry in \PB{\\{kinx}} is, likewise, bipartite: \PB{\\{addr}} is the address
in \PB{\\{kmem}}
where the list of clauses numbers for a given literal begins, and \PB{\\{size}}
is
the current length of that list. If \PB{\|l} is a free literal (namely a
literal
whose value has not been assigned true or false), \PB{$\\{kinx}[\|l].\\{size}$}
will be the
number of clauses that contain~\PB{\|l} and are not yet satisfied.
...
\PB{\\{bstack}}, whose entries are pairs of literals.
...
\&{uint} \|u${},\39\|v{}$;\C{ a pair of literals }\2\6
${}\}{}$ \&{tpair};\C{ one octabyte }\par
\fi
...
\&{int} \\{rptr}${},\39\\{iptr},\39\\{lptr}{}$;\C{ initial values of stack
pointers }\2\6
${}\}{}$ \&{ndata};\par
\fi
...
\1\1\&{void} \\{print\_bimp}(\&{int} \|l)\2\2\6
${}\{{}$\1\6
\&{register} \&{uint} \\{la}${},\39\\{ls};{}$\7
...
\\{printf}(\.{"\\n"});\6
\4${}\}{}$\2\par
\As30\*, 31\*, 33, 50, 61\ETs154.
\U2\*.\fi

\M{30\*}Similarly, the current data for big clauses gives useful diagnostic
info.

\Y\B\4\X29:Subroutines\X${}\mathrel+\E{}$\6
\1\1\&{void} \\{print\_clause}(\&{int} \|c)\2\2\6
${}\{{}$\1\6
\&{register} \&{uint} \\{la}${},\39\\{ls};{}$\7
${}\\{printf}(\.{""}\|O\.{"d:"},\39\|c);{}$\6
\&{for} ${}(\\{la}\K\\{cinx}[\|c].\\{addr};{}$ ${}\\{la}<\\{cinx}[\|c-\T{1}].%
\\{addr};{}$ ${}\\{la}\PP){}$\1\5
${}\\{printf}(\.{"\ "}\|O\.{"s"}\|O\.{".8s"}\|O\.{"s"},\39\\{litname}(\\{cmem}[%
\\{la}]),\39\\{isfree}(\\{cmem}[\\{la}])\?\.{""}:\\{iscontrary}(\\{cmem}[%
\\{la}])\?\.{"-"}:\.{"+"});{}$\2\6
${}\\{printf}(\.{"\ ("}\|O\.{"d)\\n"},\39\\{cinx}[\|c].\\{size});{}$\6
\4${}\}{}$\2\7
\1\1\&{void} \\{print\_kinx}(\&{int} \|l)\2\2\6
${}\{{}$\1\6
\&{register} \&{uint} \\{la}${},\39\\{ls};{}$\7
${}\\{printf}(\.{"kinx["}\|O\.{"s"}\|O\.{".8s]:"},\39\\{litname}(\|l));{}$\6
\&{for} ${}(\\{la}\K\\{kinx}[\|l].\\{addr},\39\\{ls}\K\\{kinx}[\|l].%
\\{size};{}$ \\{ls}; ${}\\{la}\PP,\39\\{ls}\MM){}$\1\5
${}\\{printf}(\.{"\ "}\|O\.{"d"},\39\\{kmem}[\\{la}]);{}$\2\6
\\{printf}(\.{"\\n"});\6
\4${}\}{}$\2\7
\1\1\&{void} \\{print\_full\_kinx}(\&{int} \|l)\2\2\6
${}\{{}$\1\6
\&{register} \&{uint} \\{la}${},\39\|k;{}$\7
${}\\{printf}(\.{"kinx["}\|O\.{"s"}\|O\.{".8s]:"},\39\\{litname}(\|l));{}$\6
\&{for} ${}(\\{la}\K\\{kinx}[\|l].\\{addr},\39\|k\K\T{0};{}$ ${}\|k<\\{kinx}[%
\|l].\\{size};{}$ ${}\|k\PP){}$\1\5
${}\\{printf}(\.{"\ "}\|O\.{"d"},\39\\{kmem}[\\{la}+\|k]);{}$\2\6
\&{if} ${}(\\{la}+\|k\I\\{kinx}[\|l-\T{1}].\\{addr}){}$\5
${}\{{}$\1\6
\\{printf}(\.{"\ \#"});\C{ show also the inactive clauses }\6
\&{for} ( ; ${}\\{la}+\|k<\\{kinx}[\|l-\T{1}].\\{addr};{}$ ${}\|k\PP){}$\1\5
${}\\{printf}(\.{"\ "}\|O\.{"d"},\39\\{kmem}[\\{la}+\|k]);{}$\2\6
\4${}\}{}$\2\6
\\{printf}(\.{"\\n"});\6
\4${}\}{}$\2\par
...
${}\{{}$\1\6
\&{register} \&{int} \|c${},\39\|j,\39\|k,\39\|l,\39\\{la},\39\\{ls},\39\|p,\39%
\|q,\39\|u,\39\|v;{}$\7
...
\X32\*:Check the sanity of \PB{\\{cinx}} and \PB{\\{cmem}}, \PB{\\{kinx}} and %
\PB{\\{kmem}}\X;\6
\4${}\}{}$\2\par
\fi

\M{32\*}A big clause $c=l_1\lor\cdots\lor l_k$ for $k\ge3$ begins unsatisfied,
...
clause $c$ becomes inactive: It disappears from the \PB{\\{kinx}} tables of
all free literals. Henceforth the elements of~$c$ will not be examined
again in~\PB{\\{cmem}} until we undo the setting of the literal that
inactivated~\PB{\|c}.
...
\PB{\\{size}} fields, so that we can validate the \PB{\\{kinx}} data.

\Y\B\4\X32\*:Check the sanity of \PB{\\{cinx}} and \PB{\\{cmem}}, \PB{\\{kinx}}
and \PB{\\{kmem}}\X${}\E{}$\6
\&{for} ${}(\|c\K\\{bclauses};{}$ \|c; ${}\|c\MM){}$\5
${}\{{}$\1\6
\&{for} ${}(\\{la}\K\\{cinx}[\|c].\\{addr},\39\|k\K\\{ls}\K\\{cinx}[\|c-\T{1}].%
\\{addr}-\\{la},\39\|j\K\T{0};{}$ \\{ls}; ${}\\{la}\PP,\39\\{ls}\MM){}$\5
${}\{{}$\1\6
${}\|l\K\\{cmem}[\\{la}];{}$\6
\&{if} (\\{isfree}(\|l))\1\5
\&{continue};\C{ neither true nor false }\2\6
\&{if} (\\{iscontrary}(\|l))\1\5
${}\|j\PP{}$;\C{ false }\2\6
\&{else}\1\5
\&{goto} \\{inactive};\C{ true }\2\6
\4${}\}{}$\2\6
\&{if} ${}(\|j\G\|k-\T{2}){}$\5
${}\{{}$\1\6
\&{if} ${}(\\{cinx}[\|c].\\{size}\I\T{2}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"ex-big\ clause\ "}\|O\.{"d\ has\ size\ "}\|O%
\.{"d!\\n"},\39\|c,\39\\{cinx}[\|c].\\{size});{}$\2\6
\&{goto} \\{inactive};\6
\4${}\}{}$\2\6
\&{if} ${}(\\{cinx}[\|c].\\{size}\I\|k-\|j){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"big\ clause\ "}\|O\.{"d\ has\ size\ "}\|O\.{"d%
\ not\ "}\|O\.{"d\\n"},\39\|c,\39\\{cinx}[\|c].\\{size},\39\|k-\|j);{}$\2\6
\&{continue};\6
\4\\{inactive}:\5
${}\\{cinx}[\|c].\\{size}\K\CM\\{cinx}[\|c].\\{size};{}$\6
\4${}\}{}$\2\6
\&{for} ${}(\|l\K\T{2};{}$ ${}\|l<\\{badlit};{}$ ${}\|l\PP){}$\1\6
\&{if} (\\{isfree}(\|l))\5
${}\{{}$\1\6
\&{for} ${}(\\{la}\K\\{kinx}[\|l].\\{addr},\39\\{ls}\K\\{kinx}[\|l].%
...
${}\{{}$\1\6
${}\|c\K\\{kmem}[\\{la}];{}$\6
\&{if} ${}((\&{int})\,\\{cinx}[\|c].\\{size}<\T{0}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"kinx["}\|O\.{"s"}\|O\.{".8s]\ includes\ activ}%
\)\.{e\ clause\ "}\|O\.{"d!\\n"},\39\\{litname}(\|l),\39\|c);{}$\2\6
\4${}\}{}$\2\6
\&{for} ( ; ${}\\{la}<\\{kinx}[\|l-\T{1}].\\{addr};{}$ ${}\\{la}\PP){}$\5
${}\{{}$\1\6
${}\|c\K\\{kmem}[\\{la}];{}$\6
\&{if} ${}((\&{int})\,\\{cinx}[\|c].\\{size}\G\T{0}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"kinx["}\|O\.{"s"}\|O\.{".8s]\ omits\ active\
c}\)\.{lause\ "}\|O\.{"d!\\n"},\39\\{litname}(\|l),\39\|c);{}$\2\6
\4${}\}{}$\2\6
\4${}\}{}$\2\2\6
\&{for} ${}(\|c\K\\{bclauses};{}$ \|c; ${}\|c\MM){}$\1\6
\&{if} ${}((\&{int})\,\\{cinx}[\|c].\\{size}<\T{0}){}$\1\5
${}\\{cinx}[\|c].\\{size}\K\CM\\{cinx}[\|c].\\{size}{}$;\2\2\par
\U31\*.\fi
...
\Y\B\4\X29:Subroutines\X${}\mathrel+\E{}$\6
\1\1\&{void} \\{print\_state}(\&{int} \\{lev})\2\2\6
${}\{{}$\1\6
\&{register} \&{int} \|k${},\39\|r;{}$\7
...
\&{int} \\{pfx}${},\39\\{len}{}$;\C{ prefix of its first useful appearance in
the search tree }\2\6
${}\}{}$ \&{variable};\par
\fi
...
\\{cinx}}, \PB{\\{cmem}}, \PB{\\{kinx}}, and \PB{\\{kmem}} arrays in proper
format\X;\6
...
${}\\{cinx}\K{}$(\&{tdata} ${}{*})\,\\{malloc}((\\{bclauses}+\T{1})*\&{sizeof}(%
\&{tdata}));{}$\6
\&{if} ${}(\R\\{cinx}){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Oops,\ I\ can't\ alloc}\)\.{ate\ the\ cinx\
array!\\}\)\.{n"});{}$\6
${}\\{exit}({-}\T{10});{}$\6
\4${}\}{}$\2\6
${}\\{bytes}\MRL{+{\K}}(\\{bclauses}+\T{1})*\&{sizeof}(\&{tdata});{}$\6
${}\\{cmem}\K{}$(\&{uint} ${}{*})\,\\{malloc}(\\{bcells}*\&{sizeof}(%
\&{uint}));{}$\6
\&{if} ${}(\R\\{cmem}){}$\5
${}\{{}$\1\6
...
${}\\{exit}({-}\T{10});{}$\6
\4${}\}{}$\2\6
${}\\{kinx}\K{}$(\&{tdata} ${}{*})\,\\{malloc}(\\{badlit}*\&{sizeof}(%
\&{tdata}));{}$\6
\&{if} ${}(\R\\{kinx}){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Oops,\ I\ can't\ alloc}\)\.{ate\ the\ cmem\
...
${}\\{kmem}\K{}$(\&{uint} ${}{*})\,\\{malloc}(\\{bcells}*\&{sizeof}(%
\&{uint}));{}$\6
\&{if} ${}(\R\\{kmem}){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Oops,\ I\ can't\ alloc}\)\.{ate\ the\ kmem\
array!\\}\)\.{n"});{}$\6
${}\\{exit}({-}\T{10});{}$\6
\4${}\}{}$\2\6
${}\\{bytes}\MRL{+{\K}}\\{bcells}*\&{sizeof}(\&{uint}){}$;\7
...
${}\\{freeloc}\K{}$(\&{uint} ${}{*})\,\\{malloc}((\\{vars}+\T{1})*\&{sizeof}(%
\&{uint}));{}$\6
\&{if} ${}(\R\\{freeloc}){}$\5
${}\{{}$\1\6
...
\4${}\}{}$\2\6
${}\\{bytes}\MRL{+{\K}}(\\{vars}+\T{1})*\&{sizeof}(\&{uint}){}$;\7
${}\\{nstack}\K{}$(\&{ndata} ${}{*})\,\\{malloc}((\\{vars}+\T{1})*\&{sizeof}(%
\&{ndata}));{}$\6
\&{if} ${}(\R\\{nstack}){}$\5
${}\{{}$\1\6
...
\|l].\\{istamp}\K\T{0}{}$;\2\7
${}\\{vmem}\K{}$(\&{variable} ${}{*})\,\\{malloc}((\\{vars}+\T{1})*\&{sizeof}(%
\&{variable}));{}$\6
\&{if} ${}(\R\\{vmem}){}$\5
${}\{{}$\1\6
...
\M{40\*}\B\X40\*:Copy all the temporary cells to the \PB{\\{bimp}}, \PB{%
\\{mem}}, \PB{\\{cinx}}, \PB{\\{cmem}}, \PB{\\{kinx}}, and \PB{\\{kmem}} arrays
in proper format\X${}\E{}$\6
$\\{forcedlits}\K\T{0},\39\\{cs}\K\\{proto\_truth}{}$;\C{ prepare for possible
unary clauses }\6
\&{for} ${}(\|l\K\T{2};{}$ ${}\|l<\\{badlit};{}$ ${}\|l\PP){}$\1\5
${}\|o,\39\\{kinx}[\|l].\\{addr}\K\\{kinx}[\|l].\\{size}\K\T{0}{}$;\C{ clear
the counts }\2\6
\&{for} ${}(\|c\K\\{clauses},\39\|k\K\T{0},\39\\{cc}\K\\{bclauses};{}$ \|c; ${}%
\|c\MM){}$\5
${}\{{}$\1\6
${}\\{la}\K\|k;{}$\6
...
\4${}\}{}$\2\6
${}\\{cinx}[\T{0}].\\{addr}\K\|k;{}$\6
\&{if} ${}(\|k\I\\{bcells}\V\\{cc}){}$\1\5
\\{confusion}(\.{"cmem"});\2\6
\X44\*:Build \PB{\\{kinx}} and \PB{\\{kmem}} from the stored big clauses\X;\6
\&{if} (\\{out\_file})\1\5
...
${}\|o,\39\\{cmem}[\|k\PP]\K\|p+\T{2},\39\|j\PP;{}$\6
${}\\{oo},\39\\{kinx}[\|p+\T{2}].\\{size}\PP;{}$\6
\4${}\}{}$\2\6
\&{if} (\\{out\_file})\5
${}\{{}$\1\6
...
\&{if} ${}(\|j<\T{3}){}$\5
${}\{{}$\C{ not big }\1\6
${}\|k\K\\{la},\39\|u\K\\{cmem}[\\{la}];{}$\6
${}\\{oo},\39\\{kinx}[\|u].\\{size}\MM;{}$\6
\&{if} ${}(\|j\E\T{2}){}$\5
${}\{{}$\1\6
${}\\{oo},\39\|v\K\\{cmem}[\\{la}+\T{1}],\39\\{kinx}[\|v].\\{size}\MM;{}$\6
\X43:Store a binary clause in \PB{\\{bimp}}\X;\6
\4${}\}{}$\5
\2\&{else}\1\5
\X42\*:Store a unary clause in \PB{\\{forcedlit}}\X;\2\6
\4${}\}{}$\5
\2\&{else}\1\5
${}\|o,\39\\{cinx}[\\{cc}].\\{addr}\K\\{la},\39\\{cinx}[\\{cc}].\\{size}\K\|k-%
\\{la},\39\\{cc}\MM{}$;\2\par
\U40\*.\fi
...
array.
The \PB{\\{addr}} fields in \PB{\\{kinx}} are borrowed here, temporarily, so
that no variable
is forced twice.

\Y\B\4\X42\*:Store a unary clause in \PB{\\{forcedlit}}\X${}\E{}$\6
${}\{{}$\1\6
\&{if} ${}(\|o,\39\\{kinx}[\|u].\\{addr}\E\T{0}){}$\5
${}\{{}$\1\6
\&{if} ${}(\|o,\39\\{kinx}[\\{bar}(\|u)].\\{addr}){}$\5
${}\{{}$\1\6
\&{if} ${}(\\{verbose}\AND\\{show\_choices}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"Unary\ clause\ "}\|O\.{"d\ contradicts\ unary}%
\)\.{\ clause\ "}\|O\.{"d\\n"},\39\|c,\39\\{kinx}[\\{bar}(\|u)].\\{addr});{}$\2%
\6
\&{goto} \\{unsat};\6
\4${}\}{}$\2\6
${}\|o,\39\\{kinx}[\|u].\\{addr}\K\|c;{}$\6
...
\M{44\*}\B\X44\*:Build \PB{\\{kinx}} and \PB{\\{kmem}} from the stored big
clauses\X${}\E{}$\6
$\\{max\_use}\K\T{0};{}$\6
\&{for} ${}(\|j\K\T{0},\39\|l\K\\{badlit}-\T{1};{}$ ${}\|l\G\T{2};{}$ ${}\|l%
\MM){}$\5
${}\{{}$\1\6
${}\\{oo},\39\\{kinx}[\|l].\\{addr}\K\|j,\39\\{jj}\K\\{kinx}[\|l].\\{size},\39%
\|j\MRL{+{\K}}\\{jj},\39\\{kinx}[\|l].\\{size}\K\T{0};{}$\6
\&{if} ${}(\\{jj}>\\{max\_use}){}$\1\5
${}\\{max\_use}\K\\{jj};{}$\2\6
\4${}\}{}$\2\6
${}\|o,\39\\{kinx}[\|l].\\{addr}\K\|j{}$;\C{ we'll have \PB{$\\{kinx}[\|l].%
\\{addr}+\\{kinx}[\|l].\\{size}\K\\{kinx}[\|l-\T{1}].\\{addr}$} }\6
\&{if} ${}(\|j\I\\{bcells}){}$\1\5
\\{confusion}(\.{"kinx1"});\2\6
\&{for} ${}(\|c\K\\{bclauses},\39\|j\K\T{0};{}$ \|c; ${}\|c\MM){}$\5
${}\{{}$\1\6
\&{for} ${}(\|o,\39\|k\K\\{cinx}[\|c].\\{size};{}$ \|k; ${}\|k\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\|j\PP];{}$\6
${}\|o,\39\\{la}\K\\{kinx}[\|u].\\{addr},\39\\{ls}\K\\{kinx}[\|u].\\{size};{}$\6
${}\|o,\39\\{kmem}[\\{la}+\\{ls}]\K\|c;{}$\6
${}\|o,\39\\{kinx}[\|u].\\{size}\K\\{ls}+\T{1};{}$\6
\4${}\}{}$\2\6
\4${}\}{}$\2\6
\&{if} ${}(\|j\I\\{bcells}){}$\1\5
\\{confusion}(\.{"kinx2"});\2\6
\X45\*:Allocate \PB{\\{bstack}}\X;\par
\U40\*.\fi

\M{45\*}\B\X45\*:Allocate \PB{\\{bstack}}\X${}\E{}$\6
$\\{bstack}\K{}$(\&{tpair} ${}{*})\,\\{malloc}(\\{max\_use}*\&{sizeof}(%
\&{tpair}));{}$\6
\&{if} ${}(\R\\{bstack}){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Oops,\ I\ can't\ alloc}\)\.{ate\ the\ bstack\
array}\)\.{!\\n"});{}$\6
${}\\{exit}({-}\T{10});{}$\6
\4${}\}{}$\2\6
${}\\{bytes}\MRL{+{\K}}\\{max\_use}*\&{sizeof}(\&{tpair}){}$;\par
\U44\*.\fi
...
\Y\B\4\X29:Subroutines\X${}\mathrel+\E{}$\6
\1\1\&{void} \\{resize}(\&{register} \&{int} \|l)\2\2\6
${}\{{}$\1\6
\&{register} \&{uint} \|a${},\39\|j,\39\|k,\39\\{kk},\39\|n,\39\|p,\39\|q,\39%
\|r,\39\|s;{}$\7
...
\\{level};{}$\6
\X123\*:Look ahead and gather data about how to make the next branch; but \PB{%
...
\4\\{chooseit}:\5
\X140:Choose \PB{\\{bestlit}}, which will be the next branch tried\X;\6
...
\&{uint} \\{look\_cs}${},\39\\{dlook\_cs}{}$;\C{ saved values of \PB{\\{cs}} }\6
\&{int} \\{fptr}${},\39\\{eptr},\39\\{lfptr}{}$;\C{ queue pointers for
...
\Y\B\4\X29:Subroutines\X${}\mathrel+\E{}$\6
\1\1\&{void} \\{print\_truths}(\&{uint} \\{cs})\2\2\6
${}\{{}$\1\6
\&{register} \&{int} \|x;\7
...
${}\\{fprintf}(\\{stderr},\39\.{"\\n"});{}$\6
\4${}\}{}$\2\7
\1\1\&{void} \\{print\_proto\_truths}(\&{void})\2\2\6
${}\{{}$\1\6
\\{print\_truths}(\\{proto\_truth});\6
\4${}\}{}$\2\7
\1\1\&{void} \\{print\_near\_truths}(\&{void})\2\2\6
${}\{{}$\1\6
\\{print\_truths}(\\{near\_truth});\6
\4${}\}{}$\2\7
\1\1\&{void} \\{print\_real\_truths}(\&{void})\2\2\6
${}\{{}$\1\6
...
\X68\*:Propagate binary implications of \PB{\|l}; \PB{\&{goto} \\{conflict}} if
a contradiction arises\X;\6
\4\\{promote}:\5
...
\X69\*:Update data structures for the real truth of \PB{\\{ll}}; but \PB{%
\&{goto} \\{conflict}} if a contradiction arises\X;\6
\4${}\}{}$\2\6
...
Every active big clause that contains \PB{\\{ll}} or its complement is
affected:
Those with \PB{\\{ll}} itself become satisfied, while those with \PB{\\{bar}(%
\\{ll})} become
shorter.
...
\X156\*:Swap out all big clauses that contain \PB{\\{ll}}\X;\6
${}\\{tll}\K\\{bar}(\\{ll}),\39\\{bptr}\K\T{0}{}$;\C{ clear the \PB{\\{bstack}}
}\6
\X71\*:Reduce all big clauses that contain \PB{\\{tll}}; if any become binary,
swap them out and put them on \PB{\\{bstack}}\X;\6
\&{while} (\\{bptr})\5
${}\{{}$\1\6
${}\|o,\39\\{bptr}\MM,\39\|u\K\\{bstack}[\\{bptr}].\|u,\39\|v\K\\{bstack}[%
\\{bptr}].\|v;{}$\6
...
\M{71\*}When \PB{\\{tll}} becomes false in clause \PB{\|c}, we simply decrease
the size of~\PB{\|c}
by~1, without taking time to move \PB{\\{tll}} to a different place in~\PB{%
\\{cmem}}. The
first time this happens to~\PB{\|c} is, however, special: Then we also want to
mark
all of \PB{\|c}'s other literals as ``participants,'' as explained in the
preselection process below. That case can be recognized by the condition
\PB{$\\{cinx}[\|c].\\{addr}+\\{cinx}[\|c].\\{size}\K\\{cinx}[\|c-\T{1}].%
\\{addr}$}. While we're examining those
other literals, we might as well move \PB{\\{tll}} to the end of the clause.

Interesting things start to happen when all but two of \PB{\|c}'s literals
have been falsified, before any of them have become true. At that point
\PB{\|c}~becomes inactive and its remaining literals yield a new binary clause.

\Y\B\4\X71\*:Reduce all big clauses that contain \PB{\\{tll}}; if any become
binary, swap them out and put them on \PB{\\{bstack}}\X${}\E{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ ("}\|O\.{"s"}\|O\.{".8s\ out)\\n"},\39%
\\{litname}(\\{tll}));{}$\2\6
\&{for} ${}(\|o,\39\\{tla}\K\\{kinx}[\\{tll}].\\{addr},\39\\{tls}\K\\{kinx}[%
\\{tll}].\\{size};{}$ \\{tls}; ${}\\{tla}\PP,\39\\{tls}\MM){}$\5
${}\{{}$\1\6
${}\\{oo},\39\|c\K\\{kmem}[\\{tla}],\39\\{cia}\K\\{cinx}[\|c].\\{addr},\39%
\\{cis}\K\\{cinx}[\|c].\\{size};{}$\6
\&{if} ${}(\|o,\39\\{cia}+\\{cis}\E\\{cinx}[\|c-\T{1}].\\{addr}){}$\5
${}\{{}$\C{ \PB{\|c} is reduced for the first time }\1\6
\&{for} ${}(\\{ua}\K\\{cia},\39\\{su}\K\\{cis};{}$ \\{su}; ${}\\{ua}\PP,\39%
\\{su}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{ua}];{}$\6
\&{if} ${}(\|u\E\\{tll}){}$\1\5
${}\\{au}\K\\{ua};{}$\2\6
\&{else}\1\5
\X86\*:Record \PB{\\{thevar}(\|u)} as a participant\X;\2\6
\4${}\}{}$\2\6
\&{if} ${}(\|u\I\\{tll}){}$\1\5
${}\\{oo},\39\\{cmem}[\\{ua}-\T{1}]\K\\{tll},\39\\{cmem}[\\{au}]\K\|u;{}$\2\6
\4${}\}{}$\2\6
${}\|o,\39\\{cinx}[\|c].\\{size}\K\\{cis}-\T{1};{}$\6
\&{if} ${}(\\{cis}\E\T{3}){}$\5
${}\{{}$\C{ exactly two literals of \PB{\|c} are now free }\1\6
\&{for} ${}(\\{ci}\K\\{cia},\39\|v\K\\{cmem}[\\{ci}];{}$  ; ${}\\{ci}\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{ci}];{}$\6
\&{if} (\\{isfree}(\|u))\1\5
\&{break};\2\6
\4${}\}{}$\2\6
\&{if} ${}(\\{ci}\I\\{cia}){}$\1\5
${}\\{oo},\39\\{cmem}[\\{cia}]\K\|u,\39\\{cmem}[\\{ci}]\K\|v;{}$\2\6
\&{for} ${}(\\{ci}\PP;{}$  ; ${}\\{ci}\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|v\K\\{cmem}[\\{ci}];{}$\6
\&{if} (\\{isfree}(\|v))\1\5
...
\4${}\}{}$\2\6
\&{if} ${}(\\{ci}\I\\{cia}+\T{1}){}$\1\5
${}\\{ooo},\39\\{cmem}[\\{ci}]\K\\{cmem}[\\{cia}+\T{1}],\39\\{cmem}[\\{cia}+%
\T{1}]\K\|v;{}$\2\6
${}\|o,\39\\{bstack}[\\{bptr}].\|u\K\|u,\39\\{bstack}[\\{bptr}].\|v\K\|v,\39%
\\{bptr}\PP;{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ \ "}\|O\.{"s"}\|O\.{".8s->"}\|O\.{"s"}\|O%
\.{".8s|"}\|O\.{"s"}\|O\.{".8s\\n"},\39\\{litname}(\\{bar}(\\{tll})),\39%
\\{litname}(\|u),\39\\{litname}(\|v));{}$\2\6
\X158\*:Swap \PB{\|c} out of \PB{\|u}'s clause list\X;\6
${}\|u\K\|v{}$;\5
\X158\*:Swap \PB{\|c} out of \PB{\|u}'s clause list\X;\6
\4${}\}{}$\2\6
\4${}\}{}$\2\par
...
\M{72\*}When a big clause reduces to the binary clause $u\lor v$,
...
There's good news here: We don't need to unswap any of the \PB{\\{kinx}}
entries while
...
${}\\{tll}\K\\{bar}(\\{ll});{}$\6
\X83\*:Unreduce all big clauses that contain \PB{\\{tll}}; if they had become
binary, swap them back in\X;\6
\X159\*:Swap in all big clauses that contain \PB{\\{ll}}\X;\6
${}\\{freevars}\PP;{}$\6
${}\|o,\39\\{stamp}[\\{thevar}(\\{ll})]\K\T{0};{}$\6
\4${}\}{}$\2\par
\U84.\fi

\M{83\*}\B\X83\*:Unreduce all big clauses that contain \PB{\\{tll}}; if they
had become binary, swap them back in\X${}\E{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ ("}\|O\.{"s"}\|O\.{".8s\ in)\\n"},\39%
\\{litname}(\\{tll}));{}$\2\6
\&{for} ${}(\|o,\39\\{tls}\K\\{kinx}[\\{tll}].\\{size},\39\\{tla}\K\\{kinx}[%
\\{tll}].\\{addr}+\\{tls}-\T{1};{}$ \\{tls}; ${}\\{tla}\MM,\39\\{tls}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|c\K\\{kmem}[\\{tla}];{}$\6
${}\|o,\39\\{cia}\K\\{cinx}[\|c].\\{addr},\39\\{cis}\K\\{cinx}[\|c].\\{size}+%
\T{1};{}$\6
${}\|o,\39\\{cinx}[\|c].\\{size}\K\\{cis};{}$\6
\&{if} ${}(\\{cis}\E\T{3}){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{cia}]{}$;\5
\X160\*:Swap \PB{\|c} back in to \PB{\|u}'s clause list\X;\6
${}\|o,\39\|u\K\\{cmem}[\\{cia}+\T{1}]{}$;\5
\X160\*:Swap \PB{\|c} back in to \PB{\|u}'s clause list\X;\6
\4${}\}{}$\2\6
\4${}\}{}$\2\par
\U82\*.\fi
...
\4\\{dl\_contra}:\5
\X148:Recover from a double lookahead contradiction\X;\6
\4\\{contra}:\5
\X130\*:Recover from a lookahead contradiction\X;\6
...
\X153:Print the solution found\X;\6
\4${}\}{}$\2\par
\U152\*.\fi
...
\Y\B\4\X86\*:Record \PB{\\{thevar}(\|u)} as a participant\X${}\E{}$\6
${}\{{}$\1\6
${}\|x\K\\{thevar}(\|u);{}$\6
...
\&{goto} \\{satisfied};\2\6
\X97\*:Preselect a set of candidate variables for lookahead\X;\6
\X104:Determine the strong components; \PB{\&{goto} \\{look\_bad}} if there's a
contradiction\X;\6
\X117:Construct a suitable forest\X;\par
\U123\*.\fi
...
${}\\{rating}\K{}$(\&{float} ${}{*})\,\\{malloc}((\\{vars}+\T{1})*\&{sizeof}(%
\&{float}));{}$\6
\&{if} ${}(\R\\{rating}){}$\5
${}\{{}$\1\6
...
\M{92\*}An elaborate method is used in {\mc SAT11} for the case when all big
clauses
...
\M{93\*}\B\X93\*:Compute \PB{\\{sum}}, the score of \PB{\|l}\X${}\E{}$\6
${}\{{}$\1\6
\&{ullng} \\{acc};\C{ an accumulator }\7
${}\|o,\39\\{acc}\K\\{kinx}[\\{bar}(\|l)].\\{size};{}$\6
\&{for} ${}(\|o,\39\\{la}\K\\{bimp}[\|l].\\{addr},\39\\{ls}\K\\{bimp}[\|l].%
\\{size};{}$ \\{ls}; ${}\\{la}\PP,\39\\{ls}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{mem}[\\{la}];{}$\6
\&{if} (\\{isfree}(\|u))\1\5
${}\\{acc}\MRL{+{\K}}\\{kinx}[\\{bar}(\|u)].\\{size};{}$\2\6
\4${}\}{}$\2\6
${}\\{sum}\K\\{alpha}+(\&{float})\,\\{acc};{}$\6
\4${}\}{}$\2\par
\U94\*.\fi

\M{94\*}We don't actually need the individual scores $h(l)$ for each free
literal~$l$: Only the product $h(l)\mskip1mu h(\bar l)$ is used, as
our rating for each free variable~$x$.

\Y\B\4\X94\*:Compute \PB{\\{rating}[\|x]}\X${}\E{}$\6
${}\{{}$\1\6
\&{float} \|s;\7
${}\|l\K\\{poslit}(\|x);{}$\6
\X93\*:Compute \PB{\\{sum}}, the score of \PB{\|l}\X;\6
${}\|s\K\\{sum};{}$\6
${}\|l\PP;{}$\6
\X93\*:Compute \PB{\\{sum}}, the score of \PB{\|l}\X;\6
${}\\{rating}[\|x]\K\|s*\\{sum};{}$\6
...
\.{".2f\ r="}\|O\.{".4g)\\n"},\39\\{vmem}[\|x].\\{name}.\\{ch8},\39\|s,\39%
\\{sum},\39\|s*\\{sum});{}$\2\6
\4${}\}{}$\2\par
\U95\*.\fi

\M{95\*}\B\X95\*:Put the ratings in \PB{\\{rating}}\X${}\E{}$\6
\&{for} ${}(\|k\K\T{0};{}$ ${}\|k<\\{freevars};{}$ ${}\|k\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|x\K\\{freevar}[\|k];{}$\6
\X94\*:Compute \PB{\\{rating}[\|x]}\X;\6
\4${}\}{}$\2\par
\U97\*.\fi
...
\M{97\*}\B\X97\*:Preselect a set of candidate variables for lookahead\X${}\E{}$%
\6
\X95\*:Put the ratings in \PB{\\{rating}}\X;\6
${}\\{maxcand}\K(\\{level}\E\T{0}\?\\{freevars}:\\{levelcand}/\\{level});{}$\6
\&{if} ${}(\\{maxcand}<\\{mincutoff}){}$\1\5
${}\\{maxcand}\K\\{mincutoff};{}$\2\6
\X98:Put all free participants into the initial list of candidates\X;\6
\X101:Pare down the candidates to at most \PB{\\{maxcand}}\X;\par
\U87.\fi

\M{98}The next stage in this winnowing-down process tries to avoid any variable
...
\Y\B\4\X98:Put all free participants into the initial list of candidates\X${}%
\E{}$\6
...
\&{if} ${}(\\{cands}\E\T{0}){}$\5
${}\{{}$\1\6
\X99:If all clauses are satisfied, \PB{\&{goto} \\{satisfied}}\X;\6
${}\\{no\_newbies}\K\T{0};{}$\6
\&{goto} \\{init\_cand};\C{ if there are no participants, accept all comers }\6
\4${}\}{}$\2\par
\U97\*.\fi

\M{99}\B\X99:If all clauses are satisfied, \PB{\&{goto} \\{satisfied}}\X${}%
\E{}$\6
\&{for} ${}(\|j\K\T{0};{}$ ${}\|j<\\{freevars};{}$ ${}\|j\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|x\K\\{freevar}[\|j];{}$\6
${}\|l\K\\{poslit}(\|x);{}$\6
\X100\*:If \PB{\|l} implies any unsatisfied clauses, \PB{\&{goto} \\{nogood}}%
\X;\6
${}\|l\PP;{}$\6
\X100\*:If \PB{\|l} implies any unsatisfied clauses, \PB{\&{goto} \\{nogood}}%
\X;\6
...
\M{100\*}\B\X100\*:If \PB{\|l} implies any unsatisfied clauses, \PB{\&{goto} %
\\{nogood}}\X${}\E{}$\6
\&{if} ${}(\|o,\39\\{kinx}[\\{bar}(\|l)].\\{size}){}$\1\5
\&{goto} \\{nogood};\C{ all active kinxs are unsatisfied }\2\6
\&{for} ${}(\|o,\39\\{la}\K\\{bimp}[\|l].\\{addr},\39\\{ls}\K\\{bimp}[\|l].%
\\{size};{}$ \\{ls}; ${}\\{la}\PP,\39\\{ls}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{mem}[\\{la}];{}$\6
...
\M{101}At this point we've got \PB{\\{cands}} candidates in the \PB{\\{cand}}
array,
and \PB{\\{sum}} is the sum of their ratings. The next task is to eliminate
low-rated candidates, if we have too many to handle.

\Y\B\4\X101:Pare down the candidates to at most \PB{\\{maxcand}}\X${}\E{}$\6
...
\X102:Select the \PB{\\{maxcand}} best-rated candidates\X;\2\6
\&{if} ${}(\\{cands}\E\T{0}){}$\1\5
\\{confusion}(\.{"cands"});\2\par
\U97\*.\fi

\M{102}Here we make the \PB{\\{cand}} array into a heap, with low-rated
elements
...
\Y\B\4\X102:Select the \PB{\\{maxcand}} best-rated candidates\X${}\E{}$\6
${}\{{}$\1\6
...
\M{103}\B\X103:Sift \PB{\\{cand}[\|j]} up\X${}\E{}$\6
${}\{{}$\1\6
\&{register} \&{float} \|r;\6
\&{cdata} \|c;\7
...
\N{1}{104}Strong components. If the binary implication graph has a nontrivial
...
\Y\B\4\X104:Determine the strong components; \PB{\&{goto} \\{look\_bad}} if
there's a contradiction\X${}\E{}$\6
\X106:Make all vertices unseen and all arcs untagged\X;\6
\&{for} ${}(\|i\K\T{0};{}$ ${}\|i<\\{cands};{}$ ${}\|i\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|l\K\\{poslit}(\\{cand}[\|i].\\{var});{}$\6
\4\\{check\_rank}:\5
\&{if} ${}(\|o,\39\\{lmem}[\|l].\\{rank}\E\T{0}){}$\1\5
\X112:Perform a depth-first search with \PB{\|l} as root, finding the strong
...
\X105:Print the strong components\X;\2\par
\U87.\fi

\M{105}\B\X105:Print the strong components\X${}\E{}$\6
${}\{{}$\1\6
...
\M{106}Candidates are marked with \PB{\\{bstamp}} here so that they can be
distinguished
...
\Y\B\4\X106:Make all vertices unseen and all arcs untagged\X${}\E{}$\6
\X66:Bump \PB{\\{bstamp}} to a unique value\X;\6
\&{for} ${}(\|i\K\T{0};{}$ ${}\|i<\\{cands};{}$ ${}\|i\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|l\K\\{poslit}(\\{cand}[\|i].\\{var});{}$\6
...
\4${}\}{}$\2\6
\X110:Copy all the relevant arcs to \PB{\\{cand\_arc}}\X;\6
\&{for} ${}(\|i\K\T{0};{}$ ${}\|i<\\{cands};{}$ ${}\|i\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|l\K\\{poslit}(\\{cand}[\|i].\\{var});{}$\6
...
\M{107}\B\X5:Type definitions\X${}\mathrel+\E{}$\6
...
\M{108}\B\X3\*:Global variables\X${}\mathrel+\E{}$\6
...
\M{109}The number of \PB{\\{bytes}} used will be adjusted dynamically.
...
\M{110}\B\X110:Copy all the relevant arcs to \PB{\\{cand\_arc}}\X${}\E{}$\6
\&{for} ${}(\|j\K\|i\K\T{0};{}$ ${}\|i<\\{cands};{}$ ${}\|i\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|l\K\\{poslit}(\\{cand}[\|i].\\{var});{}$\6
\X111:Copy the arcs from \PB{\|l} into the \PB{\\{cand\_arc}} array\X;\6
${}\|l\PP;{}$\6
\X111:Copy the arcs from \PB{\|l} into the \PB{\\{cand\_arc}} array\X;\6
\4${}\}{}$\2\6
\4\\{arcs\_done}:\5
...
\M{111}Beware: We {\it reverse\/} the ordering here, placing an
...
\Y\B\4\X111:Copy the arcs from \PB{\|l} into the \PB{\\{cand\_arc}} array\X${}%
\E{}$\6
...
\M{112}\B\X112:Perform a depth-first search with \PB{\|l} as root, finding the
strong components of all vertices reachable from \PB{\|l}\X${}\E{}$\6
${}\{{}$\1\6
${}\|v\K\|l;{}$\6
${}\|o,\39\\{lmem}[\|l].\\{parent}\K\T{0};{}$\6
\X113:Make vertex \PB{\|v} active\X;\6
\&{do}\5
\X114:Explore one step from the current vertex \PB{\|v}, possibly moving to
another current vertex and calling it~\PB{\|v}\X\5
\&{while} ${}(\|v>\T{0});{}$\6
\4${}\}{}$\2\par
\U104.\fi

\M{113}\B\X113:Make vertex \PB{\|v} active\X${}\E{}$\6
$\|o,\39\\{lmem}[\|v].\\{rank}\K\PP\|k;{}$\6
${}\\{lmem}[\|v].\\{link}\K\\{active},\39\\{active}\K\|v;{}$\6
${}\|o,\39\\{lmem}[\|v].\\{min}\K\|v{}$;\par
\Us112\ET114.\fi

\M{114}Minor point: No mem is charged for setting \PB{$\\{lmem}[\|v].\\{min}\K%
\|u$} here,
because \PB{$\\{lmem}[\|v].\\{untagged}$} could have been set at the same time.

\Y\B\4\X114:Explore one step from the current vertex \PB{\|v}, possibly moving
...
\&{if} ${}(\|v\E\\{ll}){}$\1\5
\X115:Remove \PB{\|v} and all its successors on the active stack from the tree,
...
\M{115}When \PB{\|v} is the representative of a strong component, all vertices
of
...
\Y\B\4\D$\\{infty}$ \5
\\{badlit}\par
\Y\B\4\X115:Remove \PB{\|v} and all its successors on the active stack from the
...
\M{117}When the program below begins its work, we will have reduced the
...
\Y\B\4\X117:Construct a suitable forest\X${}\E{}$\6
\X118:Find the heights and the child/sibling links\X;\6
\X122:Construct the \PB{\\{look}} table\X;\par
\U87.\fi

\M{118}If \PB{\|u} represents a strong component we will change \PB{$\\{lmem}[%
\|u].\\{untagged}$} to a
...
\B\4\D$\\{root}$ \5
\T{1}\par
\Y\B\4\X118:Find the heights and the child/sibling links\X${}\E{}$\6
...
\M{119}The results of our oriented forest computation are placed into
...
\M{122}Here's a standard ``double order'' traversal [{\sl TAOCP\/} exercise
...
\Y\B\4\X122:Construct the \PB{\\{look}} table\X${}\E{}$\6
...
\N{1}{123\*}Looking ahead. The lookahead process has much in common with what
...
\Y\B\4\X123\*:Look ahead and gather data about how to make the next branch; but
...
\4${}\}{}$\2\6
${}\\{fl}\K\\{forcedlits},\39\\{last\_change}\K{-}\T{1},\39\\{fptr}\K%
\\{rptr};{}$\6
${}\\{base}\K\T{2};{}$\6
\&{while} (\T{1})\5
${}\{{}$\1\6
...
\\{looki}].\\{offset};{}$\6
\X126:Look ahead at consequences of \PB{\|l}, and \PB{\&{goto} \\{look\_bad}}
...
\4\\{look\_done}:\5
${}\\{cs}\K\\{near\_truth};{}$\6
\X161\*:Reset \PB{\\{fptr}} by removing unfixed literals from \PB{\\{rstack}}%
\X;\par
\U59.\fi

\M{124}The \PB{\\{base}} keeps rising during a lookahead, never decreasing
again.
We had better use 64 bits for it, so that overflow won't be overlooked
in large instances.

\Y\B\4\X3\*:Global variables\X${}\mathrel+\E{}$\6
\&{ullng} \\{base}${},\39\\{last\_base}{}$;\C{ base address for stamps with
offsets from \PB{\\{look}} }\6
\&{uint} ${}{*}\\{forcedlit}{}$;\C{ array of forced literals }\6
\&{int} \\{forcedlits}${},\39\\{fl}{}$;\C{ the number of forced literals }\6
...
\M{125}Again we want a fast way to make literals ``snap into
...
\Y\B\4\X125:Propagate binary lookahead implications of \PB{\|l}; \PB{\&{goto} %
...
\M{126}An example will make it easier to visualize the current context.
...
\Y\B\4\X126:Look ahead at consequences of \PB{\|l}, and \PB{\&{goto} \\{look%
...
\1\5
\X129:Force \PB{\\{looklit}} to be (proto) false, and complement it\X;\2\6
\4${}\}{}$\5
\2\&{else}\5
${}\{{}$\1\6
\X132\*:Update lookahead data structures for consequences of \PB{\\{looklit}};
but \PB{\&{goto} \\{contra}} if a contradiction arises\X;\6
\&{if} ${}(\\{weighted\_new\_binaries}\E\T{0}){}$\1\5
\X127:Exploit an autarky\X\2\6
\&{else}\1\5
${}\|o,\39\\{lmem}[\\{looklit}].\\{wnb}\MRL{+{\K}}\\{weighted\_new%
\_binaries};{}$\2\6
\X142:Do a double lookahead from \PB{\\{looklit}}, if that seems advisable\X;\6
\X139:Check for necessary assignments\X;\6
\4${}\}{}$\2\par
\U123\*.\fi

\M{127}Here we implement an extension of the classical ``pure literal'' rule:
...
\Y\B\4\X127:Exploit an autarky\X${}\E{}$\6
${}\{{}$\1\6
...
temporarily }\6
\X129:Force \PB{\\{looklit}} to be (proto) false, and complement it\X;\6
\4${}\}{}$\5
\2\&{else}\5
${}\{{}$\1\6
...
\2\6
\X128\*:Make \PB{\\{ll}} equivalent to \PB{\\{looklit}}\X;\6
\4${}\}{}$\2\6
\4${}\}{}$\2\par
\U126.\fi

\M{128\*}Furthermore, if \PB{$\\{lmem}[\\{looklit}].\\{wnb}$} is nonzero, we
know that we
...
We aren't allowed to upgrade the stamp value of \PB{\\{looklit}} to
the stamp value of \PB{\\{ll}}, because that would violate an important
invariant relation: Our mechanism for undoing virtual changes to large clauses
requires that the literals in~\PB{\\{rstack}} have monotonically decreasing
levels of truth.

\Y\B\4\X128\*:Make \PB{\\{ll}} equivalent to \PB{\\{looklit}}\X${}\E{}$\6
${}\{{}$\1\6
${}\|u\K\\{bar}(\\{ll});{}$\6
...
\M{129}\B\X129:Force \PB{\\{looklit}} to be (proto) false, and complement it%
\X${}\E{}$\6
${}\{{}$\1\6
...
${}\\{cs}\K\\{look\_cs};{}$\6
\4${}\}{}$\2\par
\Us126, 127, 130\*\ETs139.\fi

\M{130\*}When we get to label \PB{\\{contra}}, we execute the following
...
\Y\B\4\X130\*:Recover from a lookahead contradiction\X${}\E{}$\6
...
\M{131\*}A new breadth-first search is launched here, as we assert \PB{%
\\{looklit}}
...
\M{132\*}We've implicitly removed \PB{\\{bar}(\\{looklit})} from all of the
active clauses.
...
\Y\B\4\X132\*:Update lookahead data structures for consequences of \PB{%
...
${}\|l\K\\{looklit};{}$\6
\X125:Propagate binary lookahead implications of \PB{\|l}; \PB{\&{goto} %
\\{contra}} if a contradiction arises\X;\6
\&{while} ${}(\\{fptr}<\\{eptr}){}$\5
${}\{{}$\1\6
${}\|o,\39\\{ll}\K\\{rstack}[\\{fptr}\PP];{}$\6
\X135\*:Update lookahead data structures for the truth of \PB{\\{ll}}; but \PB{%
\&{goto} \\{contra}} if a contradiction arises\X;\6
\4${}\}{}$\2\6
\X137:Convert the windfalls to binary implications from \PB{\\{looklit}}\X;\par
\Us126\ET129.\fi

\M{133}\B\X3\*:Global variables\X${}\mathrel+\E{}$\6
...
\M{134}\B\X58:Allocate special arrays\X${}\mathrel+\E{}$\6
...
\M{135\*}Windfalls and the weighted potentials for new binaries are discovered
here,
as we ``virtually remove'' \PB{\\{bar}(\\{ll})} from the active clauses in
which it
appears.

If all but one of the literals in such a clause has now been fixed false
at the current level, we put the remaining one on \PB{\\{bstack}} for
subsequent
analysis.

A conflict arises if all literals are fixed false. In such cases we set
\PB{$\\{bptr}\K{-}\T{1}$} instead of going immediately to \PB{\\{contra}};
otherwise
backtracking would be more complicated.

\Y\B\4\X135\*:Update lookahead data structures for the truth of \PB{\\{ll}};
but \PB{\&{goto} \\{contra}} if a contradiction arises\X${}\E{}$\6
$\\{bptr}\K\T{0};{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_gory\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ ("}\|O\.{"s"}\|O\.{".8s\ lookout)\\n"},\39%
\\{litname}(\\{bar}(\\{ll})));{}$\2\6
\&{for} ${}(\|o,\39\\{tla}\K\\{kinx}[\\{bar}(\\{ll})].\\{addr},\39\\{tls}\K%
\\{kinx}[\\{bar}(\\{ll})].\\{size};{}$ \\{tls}; ${}\\{tla}\PP,\39\\{tls}\MM){}$%
\5
${}\{{}$\1\6
${}\|o,\39\|c\K\\{kmem}[\\{tla}];{}$\6
${}\|o,\39\\{la}\K\\{cinx}[\|c].\\{addr},\39\\{ls}\K\\{cinx}[\|c].\\{size}-%
\T{1};{}$\6
${}\|o,\39\\{cinx}[\|c].\\{size}\K\\{ls};{}$\6
\&{if} ${}(\\{ls}\G\T{2}){}$\1\5
${}\\{weighted\_new\_binaries}\MRL{+{\K}}\\{clause\_weight}[\\{ls}];{}$\2\6
\&{else} \&{if} ${}(\\{bptr}\G\T{0}){}$\1\5
\X136\*:Put the remaining literal of \PB{\|c} into \PB{\\{bstack}}\X;\2\6
\4${}\}{}$\2\6
\&{if} ${}(\\{bptr}<\T{0}){}$\1\5
\&{goto} \\{contra};\2\6
\&{while} (\\{bptr})\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{bstack}[\MM\\{bptr}].\|u;{}$\6
\&{if} (\\{isfixed}(\|u))\5
${}\{{}$\1\6
\&{if} (\\{iscontrary}(\|u))\1\5
\&{goto} \\{contra};\2\6
...
\M{136\*}The remaining literal may have become fixed, but not yet virtually
removed
(because it lies between \PB{\\{fptr}} and \PB{\\{eptr}} on \PB{\\{rstack}}).

\Y\B\4\X136\*:Put the remaining literal of \PB{\|c} into \PB{\\{bstack}}\X${}%
\E{}$\6
${}\{{}$\1\6
\&{for} ${}(\|o,\39\\{ua}\K\\{cinx}[\|c-\T{1}].\\{addr};{}$ ${}\\{la}<%
\\{ua};{}$ ${}\\{la}\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{la}];{}$\6
\&{if} ${}(\R\\{isfixed}(\|u)){}$\1\5
...
\.{"d]\\n"},\39\\{litname}(\\{ll}),\39\|c);{}$\2\6
\4${}\}{}$\5
\2\&{else} \&{if} (\|u)\5
${}\{{}$\1\6
${}\|o,\39\\{bstack}[\\{bptr}\PP].\|u\K\|u;{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_gory\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ \ looking\ "}\|O\.{"s"}\|O\.{".8s->"}\|O%
\.{"s"}\|O\.{".8s\ ["}\|O\.{"d]\\n"},\39\\{litname}(\\{ll}),\39\\{litname}(%
\|u),\39\|c);{}$\2\6
\4${}\}{}$\2\6
\4${}\}{}$\2\par
\U135\*.\fi

\M{137}Windfalls are analogous to the compensation resolvents we saw before.

\Y\B\4\X137:Convert the windfalls to binary implications from \PB{\\{looklit}}%
\X${}\E{}$\6
\&{if} (\\{wptr})\5
${}\{{}$\1\6
${}\\{oo},\39\\{sl}\K\\{bimp}[\\{looklit}].\\{size},\39\\{ls}\K\\{bimp}[%
\\{looklit}].\\{alloc};{}$\6
\X138:Make sure that \PB{\\{looklit}} has an \PB{\\{istack}} entry\X;\6
...
\M{138}\B\X138:Make sure that \PB{\\{looklit}} has an \PB{\\{istack}} entry%
\X${}\E{}$\6
...
\M{139}Let \PB{$\|l\K\\{looklit}$}. If our assumption that $l$ is true has
allowed us
...
\Y\B\4\X139:Check for necessary assignments\X${}\E{}$\6
...
\M{140} Now we're ready to select \PB{\\{bestlit}}, representing our guess
about
...
\Y\B\4\X140:Choose \PB{\\{bestlit}}, which will be the next branch tried\X${}%
\E{}$\6
${}\{{}$\1\6
\&{float} \\{best\_score};\7
\&{if} ${}(\\{freevars}\E\T{0}){}$\1\5
...
\N{1}{141}Double-looking ahead. Sometimes we really go out on a limb and
...
permitted iterations. That maximum number, \PB{\\{dl\_max\_iter}}, is 32 by
default,
...
\M{142}\B\X142:Do a double lookahead from \PB{\\{looklit}}, if that seems
advisable\X${}\E{}$\6
...
\\{proto\_truth}){}$\5
${}\{{}$\1\6
\X143\*:Double look ahead from \PB{\\{looklit}}; \PB{\&{goto} \\{contra}} if a
contradiction arises\X;\6
...
\M{143\*}The new settings of \PB{\\{base}}, \PB{\\{last\_base}}, and \PB{\\{dl%
\_truth}} in this step
...
\Y\B\4\X143\*:Double look ahead from \PB{\\{looklit}}; \PB{\&{goto} \\{contra}}
if a contradiction arises\X${}\E{}$\6
$\\{last\_base}\K\\{cs}+\T{2}*\\{looks}*\\{dl\_max\_iter};{}$\6
${}\\{dl\_truth}\K\\{last\_base}+\\{cs}-\\{base};{}$\6
${}\\{base}\K\\{cs};{}$\6
${}\\{cs}\K\\{dl\_truth},\39\|l\K\\{looklit},\39\\{dlooklit}\K\|l;{}$\6
${}\\{wptr}\K\T{0};{}$\6
\X149\*:Update dlookahead data structures for consequences of \PB{%
\\{dlooklit}}; but \PB{\&{goto} \\{dl\_contra}} if a contradiction arises\X;\6
\X144\*:Run through iterations of doublelook analogous to the iterations of
ordinary lookahead\X;\6
\X137:Convert the windfalls to binary implications from \PB{\\{looklit}}\X;\par
\U142.\fi

\M{144\*}The code here and in the following sections parallels the
corresponding
routines in lookahead and in the basic solver, but at an even hazier and more
tentative level---further removed from reality.

\Y\B\4\X144\*:Run through iterations of doublelook analogous to the iterations
of ordinary lookahead\X${}\E{}$\6
...
\X146:Doublelook ahead at consequences of \PB{\|l}, and \PB{\&{goto} %
...
\\{dl\_truth}} data }\6
\X162\*:Reset the doublelook \PB{\\{fptr}} by removing unfixed literals from %
\PB{\\{rstack}}\X;\par
\U143\*.\fi

\M{145}\B\X145:Propagate binary doublelookahead implications of \PB{\|l}\X${}%
\E{}$\6
\&{if} (\\{isfixed}(\|l))\5
${}\{{}$\1\6
\&{if} (\\{iscontrary}(\|l))\1\5
...
\M{146}\B\X146:Doublelook ahead at consequences of \PB{\|l}, and \PB{\&{goto} %
...
\X147:Force \PB{\\{dlooklit}} to be (dl) false, and complement it\X;\2\6
\4${}\}{}$\5
\2\&{else}\5
${}\{{}$\1\6
\X149\*:Update dlookahead data structures for consequences of \PB{%
...
\M{147}The variable \PB{\\{dl\_last\_change}}, which keeps us doublelooking,
changes only here.

\Y\B\4\X147:Force \PB{\\{dlooklit}} to be (dl) false, and complement it\X${}%
\E{}$\6
${}\{{}$\1\6
...
\M{148}When we get to label \PB{\\{dl\_contra}}, we execute the following
...
\Y\B\4\X148:Recover from a double lookahead contradiction\X${}\E{}$\6
\&{if} ${}(\\{cs}<\\{dl\_truth}){}$\5
${}\{{}$\1\6
\X147:Force \PB{\\{dlooklit}} to be (dl) false, and complement it\X;\6
\&{goto} \\{dlook\_on};\6
\4${}\}{}$\2\6
...
\M{149\*}\B\X149\*:Update dlookahead data structures for consequences of \PB{%
...
${}\\{eptr}\K\\{fptr};{}$\6
${}\|l\K\\{dlooklit};{}$\6
\X145:Propagate binary doublelookahead implications of \PB{\|l}\X;\6
\&{while} ${}(\\{fptr}<\\{eptr}){}$\5
${}\{{}$\1\6
${}\|o,\39\\{ll}\K\\{rstack}[\\{fptr}\PP];{}$\6
\X150\*:Update dlookahead data structures for the truth of \PB{\\{ll}}; but %
\PB{\&{goto} \\{dl\_contra}} if a contradiction arises\X;\6
\4${}\}{}$\2\par
\Us143\*, 146\ETs147.\fi

\M{150\*}\B\X150\*:Update dlookahead data structures for the truth of \PB{%
\\{ll}}; but \PB{\&{goto} \\{dl\_contra}} if a contradiction arises\X${}\E{}$\6
$\\{bptr}\K\T{0};{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_doubly\_gory\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ ("}\|O\.{"s"}\|O\.{".8s\ dlookout)\\n"},\39%
\\{litname}(\\{bar}(\\{ll})));{}$\2\6
\&{for} ${}(\|o,\39\\{tla}\K\\{kinx}[\\{bar}(\\{ll})].\\{addr},\39\\{tls}\K%
\\{kinx}[\\{bar}(\\{ll})].\\{size};{}$ \\{tls}; ${}\\{tla}\PP,\39\\{tls}\MM){}$%
\5
...
\T{1};{}$\6
${}\|o,\39\\{cinx}[\|c].\\{size}\K\\{ls};{}$\6
\&{if} ${}(\\{ls}<\T{2}\W\\{bptr}\G\T{0}){}$\1\5
\X151\*:Put the remaining doublelook literal of \PB{\|c} into \PB{\\{bstack}}%
\X;\2\6
\4${}\}{}$\2\6
\&{if} ${}(\\{bptr}<\T{0}){}$\1\5
\&{goto} \\{dl\_contra};\2\6
...
\M{151\*}\B\X151\*:Put the remaining doublelook literal of \PB{\|c} into \PB{%
\\{bstack}}\X${}\E{}$\6
${}\{{}$\1\6
\&{for} ${}(\|o,\39\\{ua}\K\\{cinx}[\|c-\T{1}].\\{addr};{}$ ${}\\{la}<%
\\{ua};{}$ ${}\\{la}\PP){}$\5
...
\N{1}{152\*}Doing it. Finally we just need to put the pieces of this program
together.

\Y\B\4\X152\*:Solve the problem\X${}\E{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_big\_clauses}){}$\1\5
\X155\*:Print all the big clauses to \PB{\\{stderr}}\X;\2\6
${}\\{level}\K\T{0};{}$\6
\&{if} (\\{forcedlits})\5
${}\{{}$\1\6
...
\M{153}\B\X153:Print the solution found\X${}\E{}$\6
\&{for} ${}(\|k\K\T{0};{}$ ${}\|k<\\{rptr};{}$ ${}\|k\PP){}$\5
${}\{{}$\1\6
...
\Y\B\4\X155\*:Print all the big clauses to \PB{\\{stderr}}\X${}\E{}$\6
\&{for} ${}(\|c\K\T{1};{}$ ${}\|c\Z\\{bclauses};{}$ ${}\|c\PP){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{""}\|O\.{"d:"},\39\|c){}$;\C{ show the
reference number to the user }\6
\&{for} ${}(\\{la}\K\\{cinx}[\|c].\\{addr};{}$ ${}\\{la}<\\{cinx}[\|c-\T{1}].%
\\{addr};{}$ ${}\\{la}\PP){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ "}\|O\.{"s"}\|O\.{".8s"},\39\\{litname}(%
\\{cmem}[\\{la}]));{}$\2\6
...
\M{156\*}Here I move the remaining free literals to the left of their clauses,
if at
...
\Y\B\4\X156\*:Swap out all big clauses that contain \PB{\\{ll}}\X${}\E{}$\6
\&{for} ${}(\|o,\39\\{tla}\K\\{kinx}[\\{ll}].\\{addr},\39\\{tls}\K\\{kinx}[%
\\{ll}].\\{size};{}$ \\{tls}; ${}\\{tla}\PP,\39\\{tls}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|c\K\\{kmem}[\\{tla}];{}$\6
${}\|o,\39\\{cia}\K\\{cinx}[\|c].\\{addr},\39\\{cis}\K\\{cinx}[\|c].%
\\{size};{}$\6
${}\|o,\39\\{kk}\K\\{cinx}[\|c-\T{1}].\\{addr}-\\{cia}{}$;\C{ the original size
of clause \PB{\|c} }\6
${}\\{cis}\MM{}$;\C{ this many free literals remain }\6
\&{if} ${}(\\{cis}\Z(\\{theta64}*\\{kk})\GG\T{6}){}$\1\5
\X157\*:Swap \PB{\|c} out while gathering its free literals\X\2\6
\&{else}\1\6
\&{for} ( ; \\{cis}; ${}\\{cia}\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{cia}];{}$\6
...
\M{157\*}\B\X157\*:Swap \PB{\|c} out while gathering its free literals\X${}%
\E{}$\6
${}\{{}$\1\6
\&{for} ${}(\\{ci}\K\\{cia};{}$ \\{cis}; ${}\\{cia}\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{cia}];{}$\6
\&{if} (\\{isfree}(\|u))\5
${}\{{}$\1\6
\&{if} ${}(\\{ci}\I\\{cia}){}$\1\5
${}\\{ooo},\39\|v\K\\{cmem}[\\{ci}],\39\\{cmem}[\\{ci}]\K\|u,\39\\{cmem}[%
\\{cia}]\K\|v;{}$\2\6
\X158\*:Swap \PB{\|c} out of \PB{\|u}'s clause list\X;\6
${}\\{ci}\PP,\39\\{cis}\MM;{}$\6
\4${}\}{}$\2\6
...
\M{158\*}\B\X158\*:Swap \PB{\|c} out of \PB{\|u}'s clause list\X${}\E{}$\6
${}\{{}$\1\6
\&{for} ${}(\|o,\39\\{su}\K\\{kinx}[\|u].\\{size}-\T{1},\39\\{au}\K\\{ua}\K%
\\{kinx}[\|u].\\{addr}+\\{su};{}$ \|o${},\39\\{kmem}[\\{au}]\I\|c;{}$ ${}\\{au}%
\MM){}$\1\5
;\2\6
\&{if} ${}(\\{au}\I\\{ua}){}$\1\5
${}\\{oo},\39\\{kmem}[\\{au}]\K\\{kmem}[\\{ua}],\39\\{kmem}[\\{ua}]\K\|c;{}$\2\6
${}\|o,\39\\{kinx}[\|u].\\{size}\K\\{su};{}$\6
\4${}\}{}$\2\par
\Us71\*, 156\*\ETs157\*.\fi

\M{159\*}\B\X159\*:Swap in all big clauses that contain \PB{\\{ll}}\X${}\E{}$\6
\&{for} ${}(\|o,\39\\{tls}\K\\{kinx}[\\{ll}].\\{size},\39\\{tla}\K\\{kinx}[%
\\{ll}].\\{addr}+\\{tls}-\T{1};{}$ \\{tls}; ${}\\{tla}\MM,\39\\{tls}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|c\K\\{kmem}[\\{tla}];{}$\6
\&{for} ${}(\|o,\39\\{cia}\K\\{cinx}[\|c].\\{addr},\39\\{cis}\K\\{cinx}[\|c].%
\\{size}-\T{1};{}$ \\{cis}; ${}\\{cia}\PP){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{cmem}[\\{cia}];{}$\6
\&{if} (\\{isfree}(\|u))\5
${}\{{}$\1\6
...
\M{160\*}\B\X160\*:Swap \PB{\|c} back in to \PB{\|u}'s clause list\X${}\E{}$\6
$\\{oo},\39\\{kinx}[\|u].\\{size}\PP{}$;\par
\Us83\*\ET159\*.\fi

\M{161\*}The lookahead processes need to take back all updates to big clauses
involving literals that lose their tentative values when \PB{\\{cs}} increases.

Fortunately all literals are ordered on \PB{\\{rstack}} by their
...
\Y\B\4\X161\*:Reset \PB{\\{fptr}} by removing unfixed literals from \PB{%
\\{rstack}}\X${}\E{}$\6
\&{while} ${}(\\{fptr}>\\{rptr}){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{rstack}[\\{fptr}-\T{1}];{}$\6
\&{if} (\\{isfixed}(\|u))\1\5
\&{break};\2\6
${}\\{fptr}\MM;{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_gory\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ ("}\|O\.{"s"}\|O\.{".8s\ lookin)\\n"},\39%
\\{litname}(\\{bar}(\|u)));{}$\2\6
\X163\*:Unreduce all big clauses that contained \PB{\\{bar}(\|u)} during
lookahead\X;\6
\4${}\}{}$\2\par
\Us123\*, 130\*\ETs132\*.\fi

\M{162\*}\B\X162\*:Reset the doublelook \PB{\\{fptr}} by removing unfixed
literals from \PB{\\{rstack}}\X${}\E{}$\6
\&{while} ${}(\\{fptr}>\\{rptr}){}$\5
${}\{{}$\1\6
${}\|o,\39\|u\K\\{rstack}[\\{fptr}-\T{1}];{}$\6
\&{if} (\\{isfixed}(\|u))\1\5
...
${}\\{fptr}\MM;{}$\6
\&{if} ${}(\\{verbose}\AND\\{show\_doubly\_gory\_details}){}$\1\5
${}\\{fprintf}(\\{stderr},\39\.{"\ ("}\|O\.{"s"}\|O\.{".8s\ dlookin)\\n"},\39%
\\{litname}(\\{bar}(\|u)));{}$\2\6
\X163\*:Unreduce all big clauses that contained \PB{\\{bar}(\|u)} during
lookahead\X;\6
\4${}\}{}$\2\par
...
\M{163\*}\B\X163\*:Unreduce all big clauses that contained \PB{\\{bar}(\|u)}
during lookahead\X${}\E{}$\6
\&{for} ${}(\|o,\39\\{tls}\K\\{kinx}[\\{bar}(\|u)].\\{size},\39\\{tla}\K%
\\{kinx}[\\{bar}(\|u)].\\{addr}+\\{tls}-\T{1};{}$ \\{tls}; ${}\\{tla}\MM,\39%
\\{tls}\MM){}$\5
${}\{{}$\1\6
${}\|o,\39\|c\K\\{kmem}[\\{tla}];{}$\6
${}\|o,\39\\{cis}\K\\{cinx}[\|c].\\{size}+\T{1};{}$\6
${}\|o,\39\\{cinx}[\|c].\\{size}\K\\{cis};{}$\6
\4${}\}{}$\2\par
\Us161\*\ET162\*.\fi

\M{164\*}This program uses the \PB{\\{clause\_weight}} table to estimate a
clause's
...
\Y\B\4\X3\*:Global variables\X${}\mathrel+\E{}$\6
\&{int} \\{max\_clause};\C{ length of the longest clause }\6
\&{float} ${}{*}\\{clause\_weight}{}$;\C{ weights given to each length, for $k%
\ge2$ }\par
\fi

\M{165\*}We dare not let the \PB{\\{clause\_weight}} entries become zero,
because
that would defeat the logic by which autarkies are recognized.

\Y\B\4\X58:Allocate special arrays\X${}\mathrel+\E{}$\6
$\\{clause\_weight}\K{}$(\&{float} ${}{*})\,\\{malloc}(\\{max\_clause}*%
\&{sizeof}(\&{float}));{}$\6
\&{if} ${}(\R\\{clause\_weight}){}$\5
${}\{{}$\1\6
${}\\{fprintf}(\\{stderr},\39\.{"Oops,\ I\ can't\ alloc}\)\.{ate\ the\ clause%
\_weigh}\)\.{t\ array!\\n"});{}$\6
${}\\{exit}({-}\T{10});{}$\6
\4${}\}{}$\2\6
${}\\{bytes}\MRL{+{\K}}\\{max\_clause}*\&{sizeof}(\&{float});{}$\6
${}\\{clause\_weight}[\T{2}]\K\T{1.0};{}$\6
\&{for} ${}(\|k\K\T{3};{}$ ${}\|k<\\{max\_clause};{}$ ${}\|k\PP){}$\1\5
${}\|o,\39\\{clause\_weight}[\|k]\K\\{clause\_weight}[\|k-\T{1}]*\\{gamm}+%
\T{0.01}{}$;\2\par
\fi
...
\ch 1\*, 2\*, 3\*, 4\*, 7\*, 11\*, 24\*, 25\*, 27\*, 30\*, 31\*, 32\*, 38\*, 40%
\*, 41\*, 42\*, 44\*, 45\*, 68\*, 69\*, 71\*, 72\*, 82\*, 83\*, 86\*, 91\*, 92%
\*, 93\*, 94\*, 95\*, 96\*, 97\*, 100\*, 123\*, 128\*, 130\*, 131\*, 132\*, 135%
\*, 136\*, 143\*, 144\*, 149\*, 150\*, 151\*, 152\*, 155\*, 156\*, 157\*, 158%
