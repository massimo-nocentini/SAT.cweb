...
${}\{{}$\1\6
\&{register} \&{uint} \|c${},\39\|h,\39\|i,\39\|j,\39\|k,\39\|l,\39\|p,\39\|q,%
\39\|r,\39\\{level},\39\\{parity};{}$\7
...
\N{1}{22}SAT solving, version 0. OK, now comes my hypothetical recreation of
...
\Y\B\4\X5:Type definitions\X${}\mathrel+\E{}$\6
\&{typedef} \&{struct} ${}\{{}$\1\6
\&{uint} \\{flink}${},\39\\{blink}{}$;\C{ forward and backward links for this
literal }\6
...
\M{24}Each clause is represented by a pointer to its first cell and by its
...
\M{25}If there are $n$ variables, there are $2n$ possible literals. Hence we
...
\M{28}Similarly we can print out all of the clauses that use (or originally
used)
...
${}\\{cmem}\K{}$(\&{clause} ${}{*})\,\\{malloc}((\\{clauses}+\T{1})*\&{sizeof}(%
\&{clause}));{}$\6
\&{if} ${}(\R\\{cmem}){}$\5
${}\{{}$\1\6
...
\M{34}\B\X34:Fix up the \PB{\\{blink}} fields and compute the list sizes\X${}%
\E{}$\6
...
\M{35}The backtracking scheme we will use works nicely when the literals
...
\M{36}Sometimes doubly linked lists make me feel good, even when spending 11
mems.
...
\N{1}{39}Doing it. Now comes ye olde basic backtrack.
...
\M{40}Here's where the fact that clauses are sorted really pays off.
...
\M{41}The links dance a little here.
...
\M{42}\B\X42:Backtrack to the previous level\X${}\E{}$\6
${}\{{}$\1\6
${}\\{level}\MM;{}$\6
...
\M{43}Here the dancing links protocol requires us to traverse the list
...
\M{44}\B\X44:Put variable \PB{\\{level}} back into all clauses on the
...
\M{45}\B\X45:Print the solution found\X${}\E{}$\6
